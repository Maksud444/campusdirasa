...
export default function TadarusFormsPage() {
  ...
  const isFormAvailable = () => {
    ...
    const [startHour, startMinute] = tadarusForm.startTime.split(':').map(Number);
    const [endHour, endMinute] = tadarusForm.endTime.split(':').map(Number);
    const startTimeInMinutes = startHour * 60 + startMinute;
    const endTimeInMinutes = endHour * 60 + endMinute;

    const isDayAllowed = currentDay === tadarusForm.dayNumber;
  ...
  return (
    <div className="min-h-screen bg-gray-50" dir="rtl">
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-4xl font-bold text-white mb-2">برنامج تدارس</h1>
                <p className="text-white/90 text-lg">
                  نموذج التقديم لبرنامج تدارس لجميع الصفوف - يفتح يوم الخميس من 8 مساءً إلى 10 مساءً
                </p>
              </div>
            </div>
      ...
      <div className="max-w-4xl mx-auto px-4 py-12">
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-xl p-8 mb-12">
          ...
          <div className="space-y-3 text-gray-700">
            <div className="flex items-start gap-3">
              ...
              <p>النموذج متاح فقط يوم <strong className="text-[#00d2ff]">الخميس</strong> من <strong>8 مساءً إلى 10 مساءً</strong></p>
            </div>
            <div className="flex items-start gap-3">
              <CheckCircle className="text-[#00d2ff] flex-shrink-0 mt-1" size={20} />
        ...
        >
          {/* Card Header */}
          <div className={`${tadarusForm.bgGradient} p-8 text-center relative overflow-hidden`}>
            ...
            <div className="relative z-10 mt-4">
              ...
              <h2 className="text-3xl font-bold text-white mb-2">{tadarusForm.title}</h2>
              <p className="text-white/90 text-lg mb-1">{tadarusForm.titleEn}</p>
          ...
          <div className="p-8">
              ...
              <div className="space-y-3">
                <div className="flex items-center gap-3">
                  <div className={`px-4 py-2 rounded-lg text-base font-bold ${
                    currentDate.getDay() === tadarusForm.dayNumber
                      ? 'bg-[#00d2ff] text-white ring-4 ring-cyan-300'
                      : 'bg-white text-gray-700 border-2 border-gray-200'
                  }`}>
                    {tadarusForm.allowedDay}
                  </div>
                  {currentDate.getDay() === tadarusForm.dayNumber && (
                ...
                <div className="flex items-center gap-3 text-gray-700">
                  ...
                  <span className="font-bold text-lg">
                    من {tadarusForm.startTime.replace('20', '8')} مساءً إلى {tadarusForm.endTime.replace('22', '10')} مساءً
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="mb-6 p-6 bg-yellow-50 rounded-xl border-2 border-yellow-200">
              ...
              <ul className="space-y-2">
                {tadarusForm.requirements.map((req, idx) => (
            ...
            {isAvailable ? (
              <a
                href={tadarusForm.formLink}
                target="_blank"
                rel="noopener noreferrer"
                className={`flex items-center justify-center gap-3 ${tadarusForm.bgGradient} text-white px-8 py-4 rounded-xl hover:shadow-2xl transition-all font-bold text-lg transform hover:scale-105`}
              >
            ...
            {!isAvailable && (
              <div className="mt-4 text-center p-4 bg-gradient-to-r from-gray-50 to-blue-50 rounded-xl border border-gray-200">
                <p className="text-gray-700 font-bold text-base">
                  سيفتح النموذج يوم <span className="text-[#00d2ff]">{tadarusForm.allowedDay}</span> القادم
                </p>
                <p className="text-gray-600 text-sm mt-1">
                  من 8 مساءً إلى 10 مساءً
                </p>
              </div>
            )}
          </div>
