...
export default function IqamaPDFPage() {
  ...
  const handleDownload = (pdf: PDF) => {
    ...
    link.download = `قائمة-إقامة-${pdf.className}.pdf`;
    link.target = '_blank';
  ...
  return (
    <div className="min-h-screen bg-gray-50" dir="rtl">
      ...
      <div className="bg-gradient-to-r from-emerald-500 to-teal-500 py-12 px-4">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-4xl font-bold text-white mb-2">قوائم الإقامة</h1>
              <p className="text-white/90 text-lg">جميع قوائم الإقامة الخاصة بالطلاب</p>
            </div>
          </div>
        </div>
      </div>
      ...
      {selectedPDF && <PDFViewer pdfUrl={selectedPDF.pdfUrl} title={`قائمة إقامة ${selectedPDF.className}`} onClose={handleCloseViewer} />}
    </div>
  );
}